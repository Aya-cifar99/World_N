...
public class Engine implements Serializable {
    ...
    private final int timeForGame;

    private Engine(String gameName, GameMap map, Player player, int timeForGame) {
        this.gameName = gameName;
        this.gameMap = map;
        this.player = player;
        this.currentRoom = map.getStartRoom();
        this.timeForGame = timeForGame;
        GameFileBuilder.saveGameInfo(this);
    }
    ...
    public static Engine create(String gameName, GameMap map, Player player, int timeForGame) {
        Engine newGame = new Engine(gameName, map, player, timeForGame);
        GameFileBuilder.saveGameInfo(newGame);
        return newGame;
    }

    public static Engine demmo_game() {
        GameFileBuilder.saveGameInfo(GameFileBuilder.getSavedGame("DemoGame"));
        return GameFileBuilder.getSavedGame("DemoGame");
    }
    ...
    private boolean runBackwardCommand() {
        if (this.currentRoom.lookIfDoor(Directions.getOppositeDirection(this.player.getDirection()))) {
            Door tempDoor =
                    this.currentRoom.getIfDoor(Directions.getOppositeDirection(this.player.getDirection()));
            if (tempDoor.isOpen()) this.currentRoom = tempDoor.getSideRoom();
            System.out.println("Entered new Room..");
        }
        return true;
    }
    ...
    private void runBaseCommand(char mainCommand) {
        if (mainCommand == BaseCommands.Look.asChar) {
            System.out.println(
                    "    * "
                            + this.currentRoom.look(this.player.getDirection(), this.player.getFlashlight())
                            + " *    ");
            if (!this.currentRoom.isDark(this.player.getFlashlight())) {
                runSubCommands(
                        this.currentRoom.getWallInDirection(this.player.getDirection()).getWallContent());
            }
            return;
        }
                ...
                if (this.player.getFlashlight().switchFlashlight()) {
                    System.out.println(
                            FlashLight.className()
                                    + " switched to "
                                    + ((this.player.getFlashlight().isOn() == true) ? "on" : "off"));
                    return;
                }
    ...
    private void runDoorSubOptions() {
        ...
        if (subCommand == DoorCommands.open.asChar) {
            ((Openable) this.currentRoom.getWallInDirection(this.player.getDirection()).getWallContent())
                    .open();
        }
                ...
                if (((CheckableForOpenability)
                    ...
                    if (isLinkingEndRoom(
                            (Door)
                                    this.currentRoom.getWallInDirection(this.player.getDirection()).getWallContent()))
                        wonGame();
            } else System.out.println("Player doesn't have any keys..");
        }
        if (subCommand == 'b') System.out.println("back to main menu..");
    }
    ...
    private void runCheckForChest() {
        HashMap lockedContent =
                ((CheckableForLockedContent)
                        this.currentRoom.getWallInDirection(this.player.getDirection()).getWallContent())
                        .check();
        if ((boolean) lockedContent.keySet().toArray()[0]) {
            ...
            System.out.println(
                    "chest closed "
                            + ((ArrayList<Containable>) lockedContent.get(false)).get(0).getName()
                            + " key is needed to unlock");
        }
    }
    ...
    private void runSellerSubOptions() {
        ...
        if (subCommand == SellerCommands.trade.asChar) {
            Tradable seller =
                    ((Tradable)
                            this.currentRoom.getWallInDirection(this.player.getDirection()).getWallContent());
            ArrayList<Containable> sellerItems = (ArrayList<Containable>) seller.getItems();
            if (sellerItems.size() > 0) {
                ...
                while (true) {
                    ...
                    if (commandNumber == TradingCommands.Sell.asInt) runSellProcess(seller);
                    if (commandNumber == TradingCommands.List.asInt) showItems(sellerItems);
                    if (commandNumber == TradingCommands.Finish.asInt) break;
                }
            }
    ...
    private void runBuyProccess(Tradable seller, ArrayList<Containable> sellerItems) {
        ...
        } else System.out.println("return when you have enough gold");
    }
    ...
    private void showItems(ArrayList<Containable> sellerItems) {
        ...
        for (int i = 0; i < sellerItems.size(); i++)
            System.out.println(
                    (i + 1)
                            + ") "
                            + sellerItems.get(i).getDescription()
                            + "    "
                            + sellerItems.get(i).getPrice());
        System.out.println("    ***    ");
    }
    ...
    private void showDoorSubOptions() {
        for (int i = 0; i < DoorCommands.values().length; i++)
            System.out.println(
                    "    " + DoorCommands.values()[i].asChar + " -> " + DoorCommands.values()[i]);
    ...
    private void showPaintingSubOptions() {
        for (int i = 0; i < PaintingCommands.values().length; i++)
            System.out.println(
                    "    " + PaintingCommands.values()[i].asChar + " -> " + PaintingCommands.values()[i]);
        System.out.println("\n    b -> return to main menu");
    }
    ...
    private void showMirrorSubOptions() {
        for (int i = 0; i < MirrorCommands.values().length; i++)
            System.out.println(
                    "    " + MirrorCommands.values()[i].asChar + " -> " + MirrorCommands.values()[i]);
        System.out.println("\n    b -> return to main menu");
    }
    ...
    private void showChestSubOptions() {
        for (int i = 0; i < ChestCommands.values().length; i++)
            System.out.println(
                    "    " + ChestCommands.values()[i].asChar + " -> " + ChestCommands.values()[i]);
        System.out.println("\n    b -> return to main menu");
    }
    ...
    private void showSelleSubOptions() {
        for (int i = 0; i < SellerCommands.values().length; i++)
            System.out.println(
                    "    " + SellerCommands.values()[i].asChar + " -> " + SellerCommands.values()[i]);
        System.out.println("\n    b -> return to main menu");
    }
    ...
    private void showTradeOptions() {
        ...
        for (int i = 0; i < TradingCommands.values().length; i++) {
            System.out.println(
                    TradingCommands.values()[i].asInt + 1 + " -> " + TradingCommands.values()[i]);
        }
    }
    ...
    private void displayPlayerStatus() {
        ...
        for (Key item : this.player.getPlayerKeys()) System.out.println("    " + item.getDescription());
        if (this.player.hasFlashlight()) System.out.println("    " + FlashLight.className());
